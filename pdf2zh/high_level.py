...
def download_remote_fonts(lang: str):
    lang = lang.lower()
    # Tiếng Việt đã được thêm vào noto_list ở trên, sẽ sử dụng GoNotoKurrent-Regular.ttf
    LANG_NAME_MAP = {
        ...
        **{
            ...
            for region, langs in {
                ...
                "JP": ["ja"],
                "KR": ["ko"],
                # Xóa VN khỏi mapping vì font SourceHanSerifVN-Regular.ttf không tồn tại
                # "VN": ["vi"],
            }.items()
            for la in langs
        },
    ...
    if not Path(font_path).exists():
        try:
            # Thử lấy font từ babeldoc
            font_data = get_font_and_metadata(font_name)
            if font_data is not None:
                font_path, _ = font_data
                font_path = font_path.as_posix()
            else:
                # Nếu không tìm thấy, sử dụng mặc định
                logger.warning(f"Không tìm thấy font {font_name}, sử dụng mặc định")
            ...
            # Nếu có bất kỳ lỗi nào, sử dụng mặc định
            logger.warning(f"Lỗi khi tải font: {str(e)}, sử dụng mặc định")
            font_name = "GoNotoKurrent-Regular.ttf"
            font_data = get_font_and_metadata(font_name)
            if font_data is not None:
                font_path, _ = font_data
